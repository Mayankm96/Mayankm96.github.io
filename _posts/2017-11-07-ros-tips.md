---
title: Seven Handy Lessons for ROS Users
layout: post
date:   2017-11-07
comments: true
...
Typically, `ros::spinOnce()` is used when the program has to perform certain actions other than responding to callbacks. These include when the rate at which a particular action is performed needs to be controlled. For instance, publishing data onto a topic at a particular frequency. Ommiting either `ros::spin()` or `ros::spinOnce()` would make the code behave undesirably. Deleting `ros::spin()` from a subscriber node would close the execution after a while, while removal of `ros::spinOnce()` would make it appear as if no messages are being received. Thus, utmost care must be taken while writing your ROS node.
